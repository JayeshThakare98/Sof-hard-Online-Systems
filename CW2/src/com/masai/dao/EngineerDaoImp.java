...
public class EngineerDaoImp implements EngineerDao {
	...
	public int addEngineer(EngineerBean bean) throws EngineerExecption {
		...
		try(Connection conn = DBUtil.provideConnection()){
			...
			if(x>0) {
				...
				if(rs.next())
				id = rs.getInt(1);
				else throw new EngineerExecption("Please intered Details");
			}
	...
	public ArrayList<EngineerBean> allEngineersList() throws EngineerExecption {
		...
		try(Connection conn = DBUtil.provideConnection() ){
			...
			// TODO Auto-generated catch block
			System.out.println(e.getMessage());
			throw new EngineerExecption("Invalid Engineers please try again");
		}
	...
	public int idByEnggMail(String enggMail)  throws EngineerExecption{
		try (Connection conn = DBUtil.provideConnection()){
			...
			if(rs.next())
				...
				throw new EngineerExecption("Invalid Email");
...
//	update the status  by eng 
	...
	public String updateStatusOfComplain(int complainId,String status) throws EngineerExecption {
		...
		try(Connection conn = DBUtil.provideConnection()) {
			...
			if( s>0)
				...
				throw new EngineerExecption("Invalid  Complian Id");
	...
	public String UpdatePassWord(int id, String email, String currPass, String newPass) throws EngineerExecption {
		...
		try(Connection conn = DBUtil.provideConnection()){
			...
			else throw new EngineerExecption("Please Enter  Pass,id,mail Correctly");
...
//			}else {
			...
			else 	throw new EngineerExecption("Please Enter  Mobile,id,pass Correctly");
...
//	employee query 
	...
	@Override
	public int getAssignedEngg(int complainId) throws ComplainException {
			int EngineerId = 0;
...
try(Connection conn = DBUtil.provideConnection()){
			...
			throw new ComplainException("Invalid Complain ID");
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			System.out.println(e.getMessage());
...
//	Engineer Query 
	...
	@Override
	public ComplainBean getComplain(int complainId) throws ComplainException {
